...
def get_comp_off_leave_type():
	...
	Strictly requires the Leave Type to be configured and exist in the DB.
	"""
	...
	if not frappe.db.exists("Leave Type", leave_type):
		frappe.throw(
			_("Configured Comp Off Leave Type '{0}' does not exist in Leave Type master.").format(
				leave_type
			)
		)
...
def _match_comp_off_rule(code, rules, enabled_setting=1):
	"""
	Pure function matching attendance code against a rules table.
	"""
	...
	for rule in rules:
		...
		if rule_dict.get("attendance_code") == code and cint(rule_dict.get("enabled")):
			return flt(rule_dict.get("comp_off_days", 0.0))

	return 0.0


def get_comp_off_earned(attendance, context=None):
	"""
...
def process_comp_off_for_attendance(attendance_name):
	...
	2. Acquire row lock for concurrency protection.
	3. Calculate new earning code and days BEFORE mutating ledger/allocation state.
	4. Read existing net credit from Leave Ledger Entry.
	5. If net_credited == new_earning: no-op (idempotent).
	6. If net_credited != new_earning:
	...
	# Concurrency protection: lock Attendance record
